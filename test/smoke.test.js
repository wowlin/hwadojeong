...
test('⑨ 치수선은 헬퍼 안에서만 — 인라인 치수선 금지(라벨·선 한 세트 강제)', () => {
  // 모든 치수선 box(mat: materials.dimension)는 foundationHeightDim/heightDim/lengthDim/planVDim 안에서만 그린다.
  //   현재 = 3 + 3 + (3+3) + 3(planVDim 세로선+양틱) = 15개. 헬퍼 밖에서 직접 그리면 라벨과 분리(라벨만 지워도 선이 남음)되므로 금지.
  //   ※ 헬퍼 내부 box 수를 의도적으로 바꿨다면 이 숫자도 함께 갱신할 것.
  const src = readFileSync(mainJs, 'utf8');
  const n = (src.match(/mat: materials\.dimension/g) || []).length;
  assert.equal(n, 15, `치수선 box는 헬퍼 안 15개만이어야 함 — 현재 ${n}개. 헬퍼 밖 인라인 치수선이 생기면 라벨과 분리됨(라벨+선 한 세트 위반)`);
});
