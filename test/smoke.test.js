...
test('⑦ 안방 땅 기둥 — 입체 기둥이 잠금 좌표(PILE_POS.anbang) 단일 출처를 읽는다(도면 간 어긋남 방지)', () => {
  // 말뚝기초는 제거됐지만 땅 기둥(골조) 위치는 PILE_POS.anbang 단일 출처를 그대로 읽어야 한다.
  const src = readFileSync(mainJs, 'utf8');
  // 입체 안방 기둥: PILE_POS.anbang에서(단일 출처).
  assert.match(src, /PILE_POS\.anbang\.forEach\(\(\[px, pz\], i\) => 안방썬룸\.drawGroundPost\(px, pz, i === 0\)\)/, '입체 안방 기둥은 PILE_POS.anbang에서 읽어야 함(단일 출처)');
});
...
test('⑪ 레이어 패널 — 부품별 독립 토글(완전 독립, 누적 없음) + 배치도/전체모델 프리셋', () => {
  ...
  for (const k of ['matFoundationHouse', 'firstFloorFinish', 'stair', 'extWall', 'roof', 'deck', 'sun']) {
