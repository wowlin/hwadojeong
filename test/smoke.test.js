...
// 잡는 것: 문법·빌드/임포트 무결성·용어·기하 감사·격리(⑭⑮ 파일 경계)·메모 연동(⑯⑰) 등 회귀 전반.
// 못 잡는 것: 시각 결과(위치·정렬) — 부재 상태는 `npm run check:scene`(스냅샷 diff), 화면 판단은 사용자 몫(CLAUDE.md §1).
...
import { FLOOR_RIM_W, FLOOR_JOIST_H, FLOOR_JOIST_W, FLOOR_JOIST_SPACING } from '../src/constants.js';

const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const mainJs = resolve(root, 'src/main.js');

// src/ 전체(.js, 하위 디렉토리 포함)를 이어 붙여 스캔 — 분할 후에도 소스 검사가 파일 위치와 무관하게 동작.
function readAllSrc() {
  const out = [];
  const walk = (dir) => {
...
test('⑪ 레이어 패널 — 부품별 독립 토글(완전 독립, 누적 없음) + 배치도/전체모델 프리셋', () => {
  ...
  const viewSrc = readFileSync(resolve(root, 'src/view-state.js'), 'utf8');   // view 객체는 view-state.js로 분리됨
  const src = readFileSync(resolve(root, 'src/ui.js'), 'utf8');               // PARTS·applyVisibility·프리셋은 ui.js로 분리됨
  // (1) 부품 상태는 view 객체의 독립 boolean — 핵심 부품 키 존재
  assert.match(viewSrc, /export const view = \{/, '부품 상태는 단일 view 객체(view-state.js)');
  for (const k of ['matFoundationFull', 'firstFloorFinish', 'stair', 'firstWall', 'atticExtWall', 'roof', 'deck', 'folding']) {
    assert.match(viewSrc, new RegExp(`\\b${k}:`), `view에 부품 키 ${k} 존재`);
  }
  ...
  assert.doesNotMatch(all, /function showAll\(\)/, "'전체 모델' 프리셋 showAll 제거됨");
  // (5) 옛 누적 단계 구조 금지 — STAGE_BUTTONS·effBuilding·atLeast·blankStage 모두 제거
  assert.doesNotMatch(all, /STAGE_BUTTONS|effBuilding|blankStage/, '옛 누적 단계 구조(STAGE_BUTTONS·effBuilding·blankStage) 제거');
  assert.doesNotMatch(all, /const atLeast =/, '누적 판정 atLeast 제거(부품 독립 토글)');
});
...
test('⑫ 데크 테두리 폭 — floorFrame을 실제 실행해 부재 치수를 측정(테두리만 10cm, 가운데 가로보는 5cm 불변)', () => {
  ...
  const boxes = [];
  const ff = make((o) => { boxes.push(o); return {}; }, FLOOR_RIM_W, FLOOR_JOIST_H, FLOOR_JOIST_W, FLOOR_JOIST_SPACING);   // 골조 상수 constants.js 직참조(재하드코딩 제거)
...
test('⑯ 메모 숫자 = 코드 연동 — NOTES 안 물리 치수(m·mm·㎡)는 하드코딩 금지, ${} 보간만', () => {
  ...
  const src = readFileSync(resolve(root, 'src/notes.js'), 'utf8');   // NOTES는 notes.js로 분리됨
  ...
  const ALLOW = new Set([
    ...
    '28도', '34도', '28°', '34°',   // 태양광 최적 경사대(외부 참고 범위) — 모델 부재가 아님
    '90°',                          // ㄷ자 계단 돌음 꺾임각 — 코드가 Math.PI/2로 고정한 설계 기하(가변값 아님)
  ]);
  // 단위 확장(§5-9): mm·㎡·m + cm·T·평·kW·L·도·°(J-⑦ 확정으로 각도 단위 편입).
  //   주석은 메모 출력이 아니므로 제거 후 검사. 영문 단위는 \b(식별자 속 '2L' 오탐 방지), 한글·기호 단위는 비단어라 직접 매칭.
  const notesCode = notes.replace(/\/\/.*$/gm, '');
  const found = notesCode.match(/\d+(?:\.\d+)?\s*(?:mm|cm|㎡|m|평|kW|T|L)\b|\d+(?:\.\d+)?\s*(?:도|°)/g) || [];
...
test('⑱ 미사용 export 0 — src 참조그래프에서 참조 0인 export 금지(죽은 코드 재발 차단)', () => {
  // eslint(no-unused-vars)는 export된 심볼을 못 잡는다 → 여기서 주석 제거 후 전 코퍼스(src+test) 참조 계수로 차단(§5-9).
  const files = srcFiles(resolve(root, 'src')).concat([resolve(root, 'test/smoke.test.js')]);
  const texts = files.map((f) => readFileSync(f, 'utf8'));
  const strip = (t) => t.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/gm, '');
  const corpus = texts.map(strip).join('\n');
  const dead = [];
  for (const t of texts) {
    for (const m of t.matchAll(/^export (?:const|let) (.+)$/gm)) {
      for (const n of [...m[1].matchAll(/(?:^|,\s*)(\w+)\s*=/g)].map((x) => x[1])) {
        if ((corpus.match(new RegExp('\\b' + n + '\\b', 'g')) || []).length <= 1) dead.push(n);
      }
    }
    for (const m of t.matchAll(/^export function (\w+)/gm)) {
      if ((corpus.match(new RegExp('\\b' + m[1] + '\\b', 'g')) || []).length <= 1) dead.push(m[1]);
    }
  }
  assert.equal(dead.length, 0, `참조 0인 export: ${dead.join(', ')} — 쓰든지 지우든지(죽은 코드 금지, CLAUDE.md §6)`);
});
