...
function planHDim(fixed, a, b, text) {
  lengthDim('x', fixed, a, b, text, { labelDist: 0, side: 1, y: 0.012, h: 0.002, labelLift: 0 });   // 라벨은 치수선 중앙(labelDist 0). y=0 바닥 → 어느 시점에서도 시차 없이 중앙. 깊이무시로 안 묻힘
}
...
function planVDim(fixed, a, b, text) {
  ...
  label(text, fixed, 0, (a + b) / 2, 'dim');   // 라벨은 치수선 중앙(z 가운데, x=선 위치). y=0(바닥) → 어느 시점에서도 시차 없이 선 중앙. 깊이무시로 안 묻힘
}
...
// 부채꼴 코너 계단 — 앞·왼쪽 직선 계단 사이를 부채꼴(사분원)로 연결. (1단계: 첫 계단=가장 낮은 단 발판 높이의 부채꼴 프레임만, 다리·디딤판 없음)
{
  ...
  // 첫 계단(가장 낮은 단) 부채꼴(피자조각) 테두리 — 중심(코너)에서 뻗는 곧은 변 2개 + 바깥 호. 코너 바깥 빈 사분면에만.
  // 치수는 '바깥선' 기준(중심선 아님): 곧은 변 바깥면 = 데크 모서리선(x=cx / z=cz), 호 바깥면 = 직선 1계단 바깥 반경. 막대는 모두 그 선 '안쪽'으로만 → 계단과 안 겹치고 밖으로도 안 튀어나옴.
  const Rout = 2 * tread;                                       // 바깥선 반경(직선 1계단 바깥 끝과 동일)
  const rc = Rout - bw / 2;                                     // 호 중심선 반경 → 호 바깥면 = Rout
  const aIn = Math.asin((bw / 2) / rc);                         // 막대 두께 절반만큼 축에서 들인 시작각(끝점이 축 넘어 계단에 안 걸치게)
  const frameTopY = (topY - 2 * rise) - deckFinishT;            // 직선 1계단 틀 막대 윗면과 동일 레벨
  const ex = cx + rc * Math.cos(aIn), ez = cz - rc * Math.cos(aIn);   // 곧은 변 바깥 끝 = 호 양 끝점
  골조Objects.push(box({ x: cx, z: cz - bw, w: ex - cx, d: bw, y: frameTopY - bw, h: bw, mat, cast: false }));   // +X 곧은 변(바깥면 z=cz)
  골조Objects.push(box({ x: cx, z: ez, w: bw, d: cz - ez, y: frameTopY - bw, h: bw, mat, cast: false }));        // −Z 곧은 변(바깥면 x=cx)
  const Narc = 8;
  for (let j = 0; j < Narc; j += 1) {
    const a0 = aIn + j * (Math.PI / 2 - 2 * aIn) / Narc, a1 = aIn + (j + 1) * (Math.PI / 2 - 2 * aIn) / Narc;
    barXZ(...P(rc, a0), ...P(rc, a1), frameTopY);               // 바깥 호(중심선 rc, 바깥면 Rout)
  }
}
