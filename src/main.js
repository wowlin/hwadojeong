...
// 데크 계단틀 — 앞쪽(−Z)·왼쪽(高X) 계단을 '납작한 직사각형 테두리'로 표시. 계단 윗단(데크 상단) 높이에 공중에 뜬 것처럼. 바닥틀 그룹(골조Objects).
{
  ...
  // 밟는 표면(포세린 윗면) 균등 단차: 지면(바닥)→1계단→2계단→데크표면 = 3구간 등분.
  const deckSurfaceY = deckTopY0 + FLOOR_JOIST_H + deckFinishT;   // 데크 밟는 표면(0.70)
  const stepRise = (deckSurfaceY - groundTopY) / 3;              // 지면~데크 3등분 = 각 계단 단높이
  const step1SurfaceY = groundTopY + stepRise;                  // 1계단 밟는 표면(지면 다음, 낮은 단)
  const step2SurfaceY = groundTopY + 2 * stepRise;              // 2계단 밟는 표면(데크 직전, 높은 단)
  ...
  flatRectFrame(dXa, dXb, dZa - run, dZa, step2SurfaceY);          // 앞쪽: 데크 바로 옆 단 = 2계단(높음)
  flatRectFrame(dXa, dXb, dZa - 2 * run, dZa - run, step1SurfaceY);// 앞쪽: 한단 더 바깥 = 1계단(낮음)
  flatRectFrame(dXb, dXb + run, dZa, dZb, step2SurfaceY);          // 왼쪽: 데크 바로 옆 단 = 2계단(높음)
  flatRectFrame(dXb + run, dXb + 2 * run, dZa, dZb, step1SurfaceY);// 왼쪽: 한단 더 바깥 = 1계단(낮음)
}
