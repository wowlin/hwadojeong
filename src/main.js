...
(() => {
  ...
  captureInto(s2Wall2Objects, () => {
    ...
    const wcZ = inZ1 - 0.6;                                    // 화장실 低X 내벽(홈리프트쪽) — 뒤쪽, 문 스윙 밖
    wallOutlet(wcFaceX, wcZ, oy, '+X');
    label('화장실 콘센트', wcFaceX + 0.2, oy + 0.25, wcZ, 'mep');
  });
  ...
  captureInto(s2Wall3Objects, () => {
    ...
    const wcZ = (liftZ0 + inZ1) / 2;                                 // 3층 화장실 低X 내벽(홈리프트 뒤 벽) — 변기·세면대 반대편
    wallOutlet(inX1 - wcW3, wcZ, oy, '+X');
    label('화장실 콘센트', (inX1 - wcW3) + 0.2, oy + 0.25, wcZ, 'mep');
    const corZ = (inZ0 + RM_L) + 0.5;                               // 복도(게스트룸2 뒤~홈리프트 앞 1.0m) 안방측 외벽
