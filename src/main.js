...
(() => {
  ...
  captureInto(s2Wall3Objects, () => {
    ...
    // 실외기실 콘센트 — 高Z 벽(화장실 앞벽) 실외기 옆. 방수형(옥외 IP54)·실외기 2대 적층 대비 상하 2구(바닥 물기 피해 높이 올림)
    const ecuRoomZ = liftZ0 + 0.4;                                 // 실외기실 高Z 벽 = 화장실 앞벽(단일 출처, 앞벽 0.4m 들임)
    const ecuRoomX = inX1 - 0.4;                                   // 실외기실 깊이 중앙
    wallOutlet(ecuRoomX, ecuRoomZ, levels[2] + 0.9, '-Z');        // 실외기 1대(하단)
    wallOutlet(ecuRoomX, ecuRoomZ, levels[2] + 1.7, '-Z');        // 실외기 2대 적층(상단)
  });
...
//   층 경계(슬래브 밑면)에서 끊어 그려, 인접 층 외벽이 위·아래로 정확히 맞물림(켜면 연결).
{
  ...
  captureInto(s2Wall3Objects, () => {                                     // 3층 외벽 — 3층 슬래브 밑면~처마/용마루(박공)
    ...
    const ecuLouP0 = ecuNicheCz - 0.60, ecuLouP1 = ecuNicheCz + 0.60, ecuLouSill = lvl3 + 0.15, ecuLouHead = lvl3 + 1.10;
    const ecuLou2Sill = lvl3 + 1.30, ecuLou2Head = lvl3 + 2.30;   // 상단 방열 루버 — 하단 위 멀리언 0.20(후레싱 여유), 위끝은 외벽 최저 2.4m 밑 인방(2단, 수직벽 채움)
    ...
    label('실외기 방열 루버 1.2m폭 2단', s2W + 0.1, (ecuLouSill + ecuLouHead) / 2, ecuNicheCz, 'opening');
