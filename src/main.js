...
const floorFrameDimObjects = []; // 바닥틀 방별 너비(뒤편)·깊이(오른쪽) 치수 — 바닥틀 뷰에서만
...
const planOnlyDimObjects = [];  // 평면 전용 치수(측백 0.5m) — 바닥에만, 기초 뷰에선 숨김
...
materials.porcelainDeck = new THREE.MeshLambertMaterial({ map: makePorcelainDeckTexture() });   // 텍스처 생성기는 ./textures.js
...
function systemPile(cx, cz, headTopY, cast = false, headMat = materials.pileHead) {
...
const siteBaseObjects = [];   // 바탕 대지(흙)·도로 — 미검토 단계(1층·다락·지붕)에선 숨겨 빈 화면
...
const 골조Objects = [];          // 골조 버튼 전용 — 사용자가 단계적으로 추가할 골조 형상(스틸/목골조와 별개). 켜짐 시 입체 뷰에 표시.
...
// 데크 계단틀 — 앞쪽(−Z)·왼쪽(高X) 계단을 '납작한 직사각형 테두리'로 표시. 계단 윗단(데크 상단) 높이에 공중에 뜬 것처럼. 바닥틀 그룹(골조Objects).
{
  ...
  const nSteps = Math.max(1, Math.ceil((sTopY - groundTopY) / 0.17));
  const run = (nSteps - 1) * tread;                 // 계단 수평 깊이(디딤 합)
  ...
  flatRectFrame(living썬룸.dX0, living썬룸.dX1, living썬룸.dFrontZ - run, living썬룸.dFrontZ);   // 앞쪽 계단: 데크폭(X) × 깊이(run, −Z)
  flatRectFrame(living썬룸.dX1, living썬룸.dX1 + run, living썬룸.dFrontZ, living썬룸.dWallZ);    // 왼쪽 계단: 깊이(run, +X) × 데크깊이(Z)
}
