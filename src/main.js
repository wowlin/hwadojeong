...
// 데크 계단틀 — 앞쪽(−Z)·왼쪽(高X) 계단을 '납작한 직사각형 테두리'로 표시. 계단 윗단(데크 상단) 높이에 공중에 뜬 것처럼. 바닥틀 그룹(골조Objects).
{
  ...
  const flatRectFrame = (x0, x1, z0, z1, surfaceY) => {   // 밟는 표면 surfaceY에 맞춰 4변 틀 + 윗면 포세린 한 단
    ...
    골조Objects.push(box({ x: x0, z: z0, w: xw, d: zw, y: surfaceY - deckFinishT, h: deckFinishT, mat: materials.porcelainDeck, cast: false }));   // 틀 윗면에 2cm 포세린타일(윗면 = surfaceY)
  };
  골조Objects.push(box({ x: df.x, z: df.z, w: df.w, d: df.d, y: deckTopY0 + FLOOR_JOIST_H, h: deckFinishT, mat: materials.porcelainDeck, cast: false }));   // 데크 바닥틀 윗면에 2cm 포세린타일
