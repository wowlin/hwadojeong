...
function planDim(axis, fixed, a, b, text, labelSide = -1, labelDist = 0.6) {
  lengthDim(axis, fixed, a, b, text, { side: labelSide, labelDist: 0, y: 0.012, h: 0.002, labelPos: axis === 'z' ? 'end' : 'center' });   // 배치도 치수선 — 바닥에 붙임, 두께 2mm. 가로=선 가운데 위, 세로=선 위쪽 끝(머리 위).
}
...
// 데크 계단틀 — 앞쪽(−Z)·왼쪽(高X) 계단을 '납작한 직사각형 테두리'로 표시. 계단 윗단(데크 상단) 높이에 공중에 뜬 것처럼. 바닥틀 그룹(골조Objects).
{
  ...
  // 정면 계단 다리 — ① 지면에 2단 전체 외곽 사각틀, ② 가장 낮은 단 바깥 변 좌·우 끝 기둥 2개, ③ 데크 옆 단 앞·뒤 양끝 기둥 4개.
  {
    ...
    골조Objects.push(box({ x: gx0, z: z2f, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckFloorFrame, cast: false }));        // 2계단 앞쪽 왼끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: z2f, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckFloorFrame, cast: false })); // 2계단 앞쪽 오른끝 기둥
    골조Objects.push(box({ x: gx0, z: z2b, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckFloorFrame, cast: false }));        // 2계단 뒤쪽 왼끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: z2b, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckFloorFrame, cast: false })); // 2계단 뒤쪽 오른끝 기둥
  }
  ...
  // 왼쪽 계단 다리 — 정면 계단과 동일: ① 지면에 2단 전체 외곽 사각틀, ② 가장 낮은 단 바깥 변 앞·뒤 끝 기둥 2개, ③ 데크 옆 단 양변 앞·뒤 끝 기둥 4개.
  {
    ...
    const gxw = gx1 - gx0, gzw = gz1 - gz0;
    골조Objects.push(box({ x: gx0, z: gz0, w: t, d: gzw, y: gy, h: t, mat: materials.deckFloorFrame, cast: false }));      // 데크쪽 변(x0)
    골조Objects.push(box({ x: gx1 - t, z: gz0, w: t, d: gzw, y: gy, h: t, mat: materials.deckFloorFrame, cast: false }));  // 바깥 변(x1)
    골조Objects.push(box({ x: gx0, z: gz0, w: gxw, d: t, y: gy, h: t, mat: materials.deckFloorFrame, cast: false }));      // 앞 변(z0)
    골조Objects.push(box({ x: gx0, z: gz1 - t, w: gxw, d: t, y: gy, h: t, mat: materials.deckFloorFrame, cast: false }));  // 뒤 변(z1)
    const legW = 0.05;
    const colH = (step1SurfaceY - deckFinishT - t) - gy;   // 1계단 기둥: 지면 틀 → 가장 낮은 단(바깥) 틀 바닥
    골조Objects.push(box({ x: gx1 - legW, z: gz0, w: legW, d: legW, y: gy, h: colH, mat: materials.deckFloorFrame, cast: false }));        // 1계단 바깥쪽 앞끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: gz1 - legW, w: legW, d: legW, y: gy, h: colH, mat: materials.deckFloorFrame, cast: false })); // 1계단 바깥쪽 뒤끝 기둥
    const col2H = (step2SurfaceY - deckFinishT - t) - gy;   // 2계단(데크 옆) 기둥: 지면 틀 → 2계단 틀 바닥
    const x2o = dXb + run - legW, x2d = dXb;                // 2계단 바깥 변 / 데크쪽 변
    골조Objects.push(box({ x: x2o, z: gz0, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckFloorFrame, cast: false }));        // 2계단 바깥쪽 앞끝 기둥
    골조Objects.push(box({ x: x2o, z: gz1 - legW, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckFloorFrame, cast: false })); // 2계단 바깥쪽 뒤끝 기둥
    골조Objects.push(box({ x: x2d, z: gz0, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckFloorFrame, cast: false }));        // 2계단 데크쪽 앞끝 기둥
    골조Objects.push(box({ x: x2d, z: gz1 - legW, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckFloorFrame, cast: false })); // 2계단 데크쪽 뒤끝 기둥
  }
}
