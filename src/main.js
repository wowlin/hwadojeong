...
// 데크 계단틀 — 앞쪽(−Z)·왼쪽(高X) 계단을 '납작한 직사각형 테두리'로 표시. 계단 윗단(데크 상단) 높이에 공중에 뜬 것처럼. 바닥틀 그룹(골조Objects).
{
  ...
  // 정면 계단 다리 — ① 지면에 3단 전체 외곽(폭 dXa~dXb × 깊이 데크쪽~가장 바깥) 사각틀 1개, ② 가장 낮은 단 바깥 변 좌·우 끝 기둥 2개.
  {
    ...
    const legW = 0.05, colH = (step1SurfaceY - deckFinishT - t) - gy;   // 기둥 각관 5×5cm: 지면 틀 → 가장 낮은 단 틀 바닥
    골조Objects.push(box({ x: gx0, z: gz0, w: legW, d: legW, y: gy, h: colH, mat: materials.deckFloorFrame, cast: false }));        // 앞쪽 왼끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: gz0, w: legW, d: legW, y: gy, h: colH, mat: materials.deckFloorFrame, cast: false })); // 앞쪽 오른끝 기둥
  }
