...
function foundationHeightDim(x, z, y0, y1, text, labelDx = -0.55) {
  ...
  label(text, x + labelDx, (y0 + y1) / 2, z, 'dim');   // labelDx: 라벨을 어느 쪽으로 뺄지(+면 바깥)
}
...
// 데크 계단틀 — 앞쪽(−Z)·왼쪽(高X) 계단을 '납작한 직사각형 테두리'로 표시. 계단 윗단(데크 상단) 높이에 공중에 뜬 것처럼. 바닥틀 그룹(골조Objects).
{
  ...
  // 밟는 표면(포세린 윗면) 균등 단차: 지면(바닥)→1계단→2계단→3계단→데크표면 = 4구간 등분.
  const deckSurfaceY = deckTopY0 + FLOOR_JOIST_H + deckFinishT;   // 데크 밟는 표면(0.70)
  const stepRise = (deckSurfaceY - groundTopY) / 4;              // 지면~데크 4등분 = 각 계단 단높이(약 0.155m)
  const step1SurfaceY = groundTopY + stepRise;                  // 1계단 밟는 표면(지면 다음, 가장 낮은 단)
  const step2SurfaceY = groundTopY + 2 * stepRise;              // 2계단 밟는 표면(중간 단)
  const step3SurfaceY = groundTopY + 3 * stepRise;              // 3계단 밟는 표면(데크 직전, 가장 높은 단)
  const flatRectFrame = (x0, x1, z0, z1, surfaceY) => {   // 밟는 표면 surfaceY에 맞춰 4변 틀 + 윗면 포세린 한 단
    ...
    골조Objects.push(box({ x: x0, z: z0, w: xw, d: t, y: baseY, h: t, mat: materials.deckFloorFrame, cast: false }));        // z0 변
    골조Objects.push(box({ x: x0, z: z1 - t, w: xw, d: t, y: baseY, h: t, mat: materials.deckFloorFrame, cast: false }));    // z1 변
    골조Objects.push(box({ x: x0, z: z0, w: t, d: zw, y: baseY, h: t, mat: materials.deckFloorFrame, cast: false }));        // x0 변
    골조Objects.push(box({ x: x1 - t, z: z0, w: t, d: zw, y: baseY, h: t, mat: materials.deckFloorFrame, cast: false }));    // x1 변
  ...
  flatRectFrame(dXa, dXb, dZa - run, dZa, step3SurfaceY);              // 앞쪽: 데크 바로 옆 = 3계단(높음)
  flatRectFrame(dXa, dXb, dZa - 2 * run, dZa - run, step2SurfaceY);    // 앞쪽: 중간 = 2계단
  flatRectFrame(dXa, dXb, dZa - 3 * run, dZa - 2 * run, step1SurfaceY);// 앞쪽: 가장 바깥 = 1계단(낮음)
  // 정면 계단 다리 — ① 지면에 3단 전체 외곽(폭 dXa~dXb × 깊이 데크쪽~가장 바깥) 사각틀 1개, ② 가장 낮은 단 바깥 변 좌·우 끝 기둥 2개.
  {
    const gy = groundTopY;
    const gx0 = dXa, gx1 = dXb, gz0 = dZa - 3 * run, gz1 = dZa;   // 정면 3단 전체 외곽
    const gxw = gx1 - gx0, gzw = gz1 - gz0;
    골조Objects.push(box({ x: gx0, z: gz0, w: gxw, d: t, y: gy, h: t, mat: materials.deckFloorFrame, cast: false }));      // 바깥 변(z0)
    골조Objects.push(box({ x: gx0, z: gz1 - t, w: gxw, d: t, y: gy, h: t, mat: materials.deckFloorFrame, cast: false }));  // 데크쪽 변(z1)
    골조Objects.push(box({ x: gx0, z: gz0, w: t, d: gzw, y: gy, h: t, mat: materials.deckFloorFrame, cast: false }));      // 좌변(x0)
    골조Objects.push(box({ x: gx1 - t, z: gz0, w: t, d: gzw, y: gy, h: t, mat: materials.deckFloorFrame, cast: false }));  // 우변(x1)
    const legW = 0.05;
    const colH = (step1SurfaceY - deckFinishT - t) - gy;   // 1계단 기둥: 지면 틀 → 가장 낮은 단 틀 바닥
    골조Objects.push(box({ x: gx0, z: gz0, w: legW, d: legW, y: gy, h: colH, mat: materials.deckFloorFrame, cast: false }));        // 1계단 앞쪽 왼끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: gz0, w: legW, d: legW, y: gy, h: colH, mat: materials.deckFloorFrame, cast: false })); // 1계단 앞쪽 오른끝 기둥
    const col2H = (step2SurfaceY - deckFinishT - t) - gy, z2 = dZa - 2 * run;   // 2계단 기둥: 지면 틀 → 2계단 틀 바닥, 2계단 바깥 변
    골조Objects.push(box({ x: gx0, z: z2, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckFloorFrame, cast: false }));        // 2계단 앞쪽 왼끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: z2, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckFloorFrame, cast: false })); // 2계단 앞쪽 오른끝 기둥
    const col3H = (step3SurfaceY - deckFinishT - t) - gy;   // 3계단 기둥: 지면 틀 → 3계단 틀 바닥
    const z3f = dZa - run, z3b = dZa - legW;                // 3계단 앞쪽(바깥) 변 / 뒤쪽(데크쪽) 변
    골조Objects.push(box({ x: gx0, z: z3f, w: legW, d: legW, y: gy, h: col3H, mat: materials.deckFloorFrame, cast: false }));        // 3계단 앞쪽 왼끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: z3f, w: legW, d: legW, y: gy, h: col3H, mat: materials.deckFloorFrame, cast: false })); // 3계단 앞쪽 오른끝 기둥
    골조Objects.push(box({ x: gx0, z: z3b, w: legW, d: legW, y: gy, h: col3H, mat: materials.deckFloorFrame, cast: false }));        // 3계단 뒤쪽 왼끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: z3b, w: legW, d: legW, y: gy, h: col3H, mat: materials.deckFloorFrame, cast: false })); // 3계단 뒤쪽 오른끝 기둥
  }
  flatRectFrame(dXb, dXb + run, dZa, dZb, step3SurfaceY);              // 왼쪽: 데크 바로 옆 = 3계단(높음)
  flatRectFrame(dXb + run, dXb + 2 * run, dZa, dZb, step2SurfaceY);    // 왼쪽: 중간 = 2계단
  flatRectFrame(dXb + 2 * run, dXb + 3 * run, dZa, dZb, step1SurfaceY);// 왼쪽: 가장 바깥 = 1계단(낮음)
}
// 부채꼴 코너 계단 — 앞·왼쪽 계단이 만나는 코너(dX1,dFrontZ) 바깥 사분면(+X~−Z)을 방사형 단으로 연결.
{
  const cdf = deckFootprints[0];                               // 데크 footprint 기준(직선 계단과 동일 좌표)
  const cx = cdf.x + cdf.w, cz = cdf.z;                        // 코너점(앞·왼쪽 만나는 곳)
  const topY = deckTopY0 + deckFinishT, baseY = groundTopY, tread = 0.3, bw = 0.05;   // 각관 5×5cm(직선 계단과 통일)
  const nSteps = 2;                                              // 한단 크기 — 디딤 1개(반경 tread), 직선 계단과 동일
  ...
  const post = (r, a, y0, y1) => { const [x, z] = P(r, a); 골조Objects.push(box({ x: x - bw / 2, z: z - bw / 2, w: bw, d: bw, y: y0, h: y1 - y0, mat, cast: false })); };
  const barXZ = (x0, z0, x1, z1, y) => {   // 두 XZ점을 잇는 수평 바(Y축 회전)
    const dx = x1 - x0, dz = z1 - z0, len = Math.hypot(dx, dz) + bw;
    const m = new THREE.Mesh(new THREE.BoxGeometry(len, bw, bw), mat);
    m.position.set((x0 + x1) / 2, y + bw / 2, (z0 + z1) / 2);
    m.rotation.y = Math.atan2(-dz, dx);
    m.castShadow = false; m.receiveShadow = false; scene.add(m);
    골조Objects.push(m);   // 외곽선 없이 색상만
  };
  const angs = [0, Math.PI / 2];     // 방사형 스트링거는 양 끝(0·90°, 직선 계단과 잇는 줄)만 — 중간 살 없음
  for (const a of angs) {
    for (let i = 0; i < nSteps; i += 1) { if (i === 0 && a !== 0) continue; post(i * tread, a, topY - (i + 1) * rise, topY - i * rise); }   // 챌판(중심 r=0은 1번만)
    for (let i = 0; i < nSteps - 1; i += 1) { const [x0, z0] = P(i * tread, a), [x1, z1] = P((i + 1) * tread, a); barXZ(x0, z0, x1, z1, topY - (i + 1) * rise); }   // 방사 디딤
  }
  const arc = (R, y) => { const N = 6; for (let j = 0; j < N; j += 1) { const a0 = j * (Math.PI / 2) / N, a1 = (j + 1) * (Math.PI / 2) / N; const [x0, z0] = P(R, a0), [x1, z1] = P(R, a1); barXZ(x0, z0, x1, z1, y); } };
  for (let i = 0; i < nSteps - 1; i += 1) arc((i + 1) * tread, topY - (i + 1) * rise);   // 단별 호 노징
  arc((nSteps - 1) * tread, baseY);   // 발치 호
}
