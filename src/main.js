...
{
  const sTopY = deckTopY0 + deckFinishT;            // 계단 윗단(데크 상단) 높이 — 이 높이에 띄움
  ...
  const flatRectFrame = (x0, x1, z0, z1) => {       // Y=sTopY 평면에 4변 막대로 두른 납작한 테두리
    ...
    골조Objects.push(box({ x: x0, z: z0, w: xw, d: zw, y: sTopY + t, h: deckFinishT, mat: materials.porcelainDeck, cast: false }));   // 틀 윗면에 2cm 포세린타일 한 장
  };
  ...
  flatRectFrame(dXa, dXb, dZa - run, dZa);          // 앞쪽 계단 1단: 데크폭(X) × 깊이(run, −Z)
  flatRectFrame(dXa, dXb, dZa - 2 * run, dZa - run);// 앞쪽 계단 2단: 한단 더 바깥(−Z)
  flatRectFrame(dXb, dXb + run, dZa, dZb);          // 왼쪽 계단 1단: 깊이(run, +X) × 데크깊이(Z)
  flatRectFrame(dXb + run, dXb + 2 * run, dZa, dZb);// 왼쪽 계단 2단: 한단 더 바깥(+X)
}
