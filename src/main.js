...
//   main.js 전역  : label(text,x,y,z,size) · room/frontSash/sideSash/sideDoor/germanSlidingDoor/entryDoor/pocketDoor* · foundationHeightDim/planXDim · setView/applyVisibility
//
// ▌주의 (ES모듈·strict)
...
import {
  buildingW, buildingFrontZ, buildingBackZ, groundTopY, pileR,
...
import {
  buildingD, foundationTopY, firstFloorY, deckTopY0, lotX1, lotZ1,
...
const LABEL_GROUPS = {
  dim:       { bg: 'rgba(255,255,255,0.88)', size: 0.40 },   // 치수
...
function foundationHeightDim(x, z, y0, y1, text, labelDx = -0.55) {
  ...
  label(text, x, y1 + 0.5, z, 'dim');   // 라벨은 수직 치수선 위쪽 끝(y1)에서 살짝 위(0.5)
}

// 수직 높이 치수(범용) — 수직선 + 상·하 X방향 틱 + 라벨을 ★한 세트★로 묶는다(라벨·선 분리 금지: 한 줄로 추가/삭제).
function heightDim(z, y0, y1, text, { lineX, tickX, tickW = 0.35, lw = 0.035, labelX, labelZ = z, name, cast = true } = {}) {
  ...
  label(text, labelX, (y0 + y1) / 2, labelZ, 'dim');
}
...
// 평면(배치도) 가로(x축) 치수 — 라벨은 선 길이 가운데 위. 치수 카드(높이 약 0.48m)가 땅에 안 박히게 y로 띄움.
function planXDim(fixed, a, b, text) {
  lengthDim('x', fixed, a, b, text, { labelDist: 0, side: 1, y: 0.012, h: 0.002, labelLift: 0.5 });   // 라벨은 치수선 중앙(labelDist 0), y축으로 치수선보다 살짝 위(labelLift 0.5)
}
...
captureInto(floorFrameDimObjects, () => {
  const yTop = foundationTopY + FLOOR_JOIST_H;   // 바닥틀 상단(0.78)에 치수 얹음
  const backZ = frBackZ + 0.5;                   // 뒤 림장선 바깥으로 너비 치수선을 뺀다
  // 너비(가로 X) — 평면 좌(高x)→우(低x): 안방·계단실·거실. 계단실은 양벽 중심선, 안방·거실은 계단쪽 벽 중심→외곽 끝.
  lengthDim('x', backZ, 안방InnerWallX, buildingW, `안방 ${fmtDim(buildingW - 안방InnerWallX)}m`, { y: yTop, side: 1, labelDist: 0.6 });
  lengthDim('x', backZ, 거실InnerWallX, 안방InnerWallX, `계단실 ${fmtDim(안방InnerWallX - 거실InnerWallX)}m`, { y: yTop, side: 1, labelDist: 0.6 });
  lengthDim('x', backZ, 0, 거실InnerWallX, `거실 ${fmtDim(거실InnerWallX - 0)}m`, { y: yTop, side: 1, labelDist: 0.6 });
  // 깊이(세로 Z) — 오른쪽(평면 우 = 低x = 거실쪽) 외곽 바깥에. 외곽치수(footprint 앞~뒤 끝).
  lengthDim('z', -0.5, buildingFrontZ, buildingBackZ, `깊이 ${fmtDim(buildingBackZ - buildingFrontZ)}m`, { y: yTop, side: -1, labelDist: 0.6 });
...
footprintObjects.push(box({ x: lotX0, z: lotZ1 - 0.5, w: lotW, d: 0.5, y: planY, h: planH, mat: materials.hedge, cast: false, name: 'ground' }));   // 측백(후면)
footprintObjects.push(box({ x: lotX1 - 0.5, z: lotZ0, w: 0.5, d: lotD, y: planY, h: planH, mat: materials.hedge, cast: false, name: 'ground' }));   // 측백(좌측)
...
captureInto(dimObjects, () => {
  ...
  captureInto(planOnlyDimObjects, () => planXDim(lotZ1 + 0.4, lotX1 - 0.5, lotX1, '측백 0.5m'));   // 가족방 측백 0.5(좌상단) — 바닥 전용(기초 뷰 숨김)
  // 세로 — 가족방(왼쪽) 건물 깊이 4 / 거실(오른쪽) 뒤 이격 합 1m + 건물 깊이 4 + 데크 깊이
  planZDim(lotX1 + 0.35, buildingFrontZ, buildingBackZ, '4.0m');          // 가족방 건물 깊이
  captureInto(planOnlyDimObjects, () => planZDim(lotX1 + 0.35, lotZ1 - 0.5, lotZ1, '측백 0.5m'));   // 뒤(가로) 측백 0.5 — 바닥 전용(기초 뷰 숨김)
...
// 데크 계단틀 — 앞쪽(−Z)·왼쪽(高X) 계단을 '납작한 직사각형 테두리'로 표시. 계단 윗단(데크 상단) 높이에 공중에 뜬 것처럼. 바닥틀 그룹(골조Objects).
{
  ...
  const flatRectFrame = (x0, x1, z0, z1, surfaceY) => {   // 밟는 표면 surfaceY에 맞춰 4변 틀 + 윗면 포세린 한 단
    ...
    골조Objects.push(box({ x: x0, z: z0, w: xw, d: t, y: baseY, h: t, mat: materials.deckStairFrame, cast: false }));        // z0 변
    골조Objects.push(box({ x: x0, z: z1 - t, w: xw, d: t, y: baseY, h: t, mat: materials.deckStairFrame, cast: false }));    // z1 변
    골조Objects.push(box({ x: x0, z: z0, w: t, d: zw, y: baseY, h: t, mat: materials.deckStairFrame, cast: false }));        // x0 변
    골조Objects.push(box({ x: x1 - t, z: z0, w: t, d: zw, y: baseY, h: t, mat: materials.deckStairFrame, cast: false }));    // x1 변
    floorFinishObjects.push(box({ x: x0, z: z0, w: xw, d: zw, y: surfaceY - deckFinishT, h: deckFinishT, mat: materials.porcelainDeck, cast: false }));   // 틀 윗면에 2cm 포세린타일(윗면 = surfaceY) — '바닥' 단계부터 표시
  };
  ...
  // 정면 계단 다리 — ① 지면에 2단 전체 외곽 사각틀, ② 가장 낮은 단 바깥 변 좌·우 끝 기둥 2개, ③ 데크 옆 단 앞·뒤 양끝 기둥 4개.
  {
    ...
    골조Objects.push(box({ x: gx0, z: gz0, w: gxw, d: t, y: gy, h: t, mat: materials.deckStairFrame, cast: false }));      // 바깥 변(z0)
    골조Objects.push(box({ x: gx0, z: gz1 - t, w: gxw, d: t, y: gy, h: t, mat: materials.deckStairFrame, cast: false }));  // 데크쪽 변(z1)
    골조Objects.push(box({ x: gx0, z: gz0, w: t, d: gzw, y: gy, h: t, mat: materials.deckStairFrame, cast: false }));      // 좌변(x0)
    골조Objects.push(box({ x: gx1 - t, z: gz0, w: t, d: gzw, y: gy, h: t, mat: materials.deckStairFrame, cast: false }));  // 우변(x1)
    const legW = 0.05;
    const colH = (step1SurfaceY - deckFinishT - t) - gy;   // 1계단 기둥: 지면 틀 → 가장 낮은 단(바깥) 틀 바닥
    골조Objects.push(box({ x: gx0, z: gz0, w: legW, d: legW, y: gy, h: colH, mat: materials.deckStairFrame, cast: false }));        // 1계단 앞쪽 왼끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: gz0, w: legW, d: legW, y: gy, h: colH, mat: materials.deckStairFrame, cast: false })); // 1계단 앞쪽 오른끝 기둥
    const col2H = (step2SurfaceY - deckFinishT - t) - gy;   // 2계단(데크 옆) 기둥: 지면 틀 → 2계단 틀 바닥
    const z2f = dZa - run, z2b = dZa - legW;                // 2계단 앞쪽(바깥) 변 / 뒤쪽(데크쪽) 변
    골조Objects.push(box({ x: gx0, z: z2f, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckStairFrame, cast: false }));        // 2계단 앞쪽 왼끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: z2f, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckStairFrame, cast: false })); // 2계단 앞쪽 오른끝 기둥
    골조Objects.push(box({ x: gx0, z: z2b, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckStairFrame, cast: false }));        // 2계단 뒤쪽 왼끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: z2b, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckStairFrame, cast: false })); // 2계단 뒤쪽 오른끝 기둥
  }
  ...
  // 왼쪽 계단 다리 — 정면 계단과 동일: ① 지면에 2단 전체 외곽 사각틀, ② 가장 낮은 단 바깥 변 앞·뒤 끝 기둥 2개, ③ 데크 옆 단 양변 앞·뒤 끝 기둥 4개.
  {
    ...
    const gxw = gx1 - gx0, gzw = gz1 - gz0;
    골조Objects.push(box({ x: gx0, z: gz0, w: t, d: gzw, y: gy, h: t, mat: materials.deckStairFrame, cast: false }));      // 데크쪽 변(x0)
    골조Objects.push(box({ x: gx1 - t, z: gz0, w: t, d: gzw, y: gy, h: t, mat: materials.deckStairFrame, cast: false }));  // 바깥 변(x1)
    골조Objects.push(box({ x: gx0, z: gz0, w: gxw, d: t, y: gy, h: t, mat: materials.deckStairFrame, cast: false }));      // 앞 변(z0)
    골조Objects.push(box({ x: gx0, z: gz1 - t, w: gxw, d: t, y: gy, h: t, mat: materials.deckStairFrame, cast: false }));  // 뒤 변(z1)
    const legW = 0.05;
    const colH = (step1SurfaceY - deckFinishT - t) - gy;   // 1계단 기둥: 지면 틀 → 가장 낮은 단(바깥) 틀 바닥
    골조Objects.push(box({ x: gx1 - legW, z: gz0, w: legW, d: legW, y: gy, h: colH, mat: materials.deckStairFrame, cast: false }));        // 1계단 바깥쪽 앞끝 기둥
    골조Objects.push(box({ x: gx1 - legW, z: gz1 - legW, w: legW, d: legW, y: gy, h: colH, mat: materials.deckStairFrame, cast: false })); // 1계단 바깥쪽 뒤끝 기둥
    const col2H = (step2SurfaceY - deckFinishT - t) - gy;   // 2계단(데크 옆) 기둥: 지면 틀 → 2계단 틀 바닥
    const x2o = dXb + run - legW, x2d = dXb;                // 2계단 바깥 변 / 데크쪽 변
    골조Objects.push(box({ x: x2o, z: gz0, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckStairFrame, cast: false }));        // 2계단 바깥쪽 앞끝 기둥
    골조Objects.push(box({ x: x2o, z: gz1 - legW, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckStairFrame, cast: false })); // 2계단 바깥쪽 뒤끝 기둥
    골조Objects.push(box({ x: x2d, z: gz0, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckStairFrame, cast: false }));        // 2계단 데크쪽 앞끝 기둥
    골조Objects.push(box({ x: x2d, z: gz1 - legW, w: legW, d: legW, y: gy, h: col2H, mat: materials.deckStairFrame, cast: false })); // 2계단 데크쪽 뒤끝 기둥
  }
}
// 부채꼴 코너 계단 — 앞·왼쪽 직선 계단 사이를 부채꼴(사분원)로 연결. (1단계: 첫 계단=가장 낮은 단 발판 높이의 부채꼴 프레임만, 다리·디딤판 없음)
{
  ...
  const arcEndX = (R) => cx + (R - bw / 2) * Math.cos(Math.asin((bw / 2) / (R - bw / 2)));   // 반경 R 호의 +X쪽 끝 x(축 안쪽으로 들인 지점)
  const arcEndZ = (R) => cz - (R - bw / 2) * Math.cos(Math.asin((bw / 2) / (R - bw / 2)));   // 반경 R 호의 −Z쪽 끝 z
  const drawArc = (R, frameTopY) => {                           // 바깥면이 R인 호(중심선 R-bw/2), 양 끝이 축 안쪽(z=cz-bw/2 / x=cx+bw/2)에 닿게
    const r = R - bw / 2, a = Math.asin((bw / 2) / r), N = 8;
    for (let j = 0; j < N; j += 1) {
      const a0 = a + j * (Math.PI / 2 - 2 * a) / N, a1 = a + (j + 1) * (Math.PI / 2 - 2 * a) / N;
      barXZ(...P(r, a0), ...P(r, a1), frameTopY);
    }
  };
  // 부채꼴 한 단: Rin=0이면 피자조각(꼭지점까지 곧은 변), Rin>0이면 띠(안쪽 호~바깥 호, 꼭지점쪽 곧은 변 제거). 곧은 변 바깥면 = 모서리선.
  const fan = (Rout, frameTopY, Rin = 0) => {
    const ox = arcEndX(Rout), oz = arcEndZ(Rout);
    const ix = Rin > 0 ? arcEndX(Rin) : cx, iz = Rin > 0 ? arcEndZ(Rin) : cz;
    골조Objects.push(box({ x: ix, z: cz - bw, w: ox - ix, d: bw, y: frameTopY - bw, h: bw, mat, cast: false }));   // +X 곧은 변(바깥면 z=cz)
    골조Objects.push(box({ x: cx, z: oz, w: bw, d: iz - oz, y: frameTopY - bw, h: bw, mat, cast: false }));        // −Z 곧은 변(바깥면 x=cx)
    drawArc(Rout, frameTopY);                                   // 바깥 호
    if (Rin > 0) drawArc(Rin, frameTopY);                       // 안쪽 호(맨위 부채꼴 호와 동일 반경)
  };
  fan(2 * tread, baseY + bw);                                  // 계단 바닥(지면 베이스): 피자조각 부채꼴(바깥 반경 2·디딤), 지면 높이
  fan(2 * tread, (topY - 2 * rise) - deckFinishT, 1 * tread);   // 중간(첫 계단=낮은 단): 안쪽 호 1·디딤 ~ 바깥 2·디딤 '띠'(꼭지점쪽 곧은 변 제거)
  fan(1 * tread, (topY - 1 * rise) - deckFinishT);             // 맨위(둘째 계단=높은 단): 피자조각 부채꼴(바깥 반경 1·디딤)
}
