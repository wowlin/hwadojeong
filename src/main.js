...
(() => {
  ...
  captureInto(s2LiftObjects, () => drawLiftColumn(levels[0], roofY, true));   // 홈리프트 전체 샤프트(1층 바닥~처마) — 독립 '홈리프트' 토글(층 그룹과 분리)
  ...
  captureInto(s2Floor3Objects, () => {
    ...
    // 계단 올라오는 자리(최상층 — 위로 더 오를 계단 없음) — 2층 도착칸처럼 계단실 폭 그대로 뒤 외벽까지. 다른 용도 불가.
    box({ x: far3, z: zB0, w: cgcW, d: inZ1 - zB0, y: levels[2] + 0.006, h: 0.012, mat: materials.stairUpZone3, cast: false });
  ...
  captureInto(s2Wall2Objects, () => {
    ...
    wallOutlet(inX1, acOutletZ, oyAc, '-X');
...
const NOTE_ORDER = ['plan', 'foundation', 'matFoundationHouse', 'matFoundationFull', 'firstFloorFinish', 'stair', 'livingWall', 'familyWall', 'extWall', 'firstRoom', 'anno', 'outlet', 'bath', 'loft', 'roof', 'deck', 'deckFloor', 'deckStairFrame', 'sun', 'sunWall', 'folding', 'accessory', 'hedge', 'fence', 's2Foundation', 's2Floor1', 's2Stair', 's2Lift', 's2Floor2', 's2Floor3', 's2Wall3', 's2Roof3', 's2Solar3'];
