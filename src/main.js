...
(() => {
  ...
  captureInto(s2Wall3Objects, () => {
    ...
    const corX = inX1 - 0.7;                                        // 복도 앞/뒤벽 콘센트 X — 화장실문 밖·측벽 콘센트와 이격
    wallOutlet(corX, inZ0 + RM_L, oy, '+Z');                        // 앞쪽벽(게스트룸2 분리벽) — 복도로 +Z 돌출
    wallOutlet(corX, liftZ0, oy, '-Z');                             // 뒤쪽벽(홈리프트·화장실 복도벽) — 복도로 -Z 돌출
    label('복도 콘센트', corX, oy + 0.25, ((inZ0 + RM_L) + liftZ0) / 2, 'mep');
  });
