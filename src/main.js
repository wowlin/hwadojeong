...
(() => {
  ...
  captureInto(s2Floor3Objects, () => {
    ...
    const wcSetback3 = 0.2;                     // 3층 화장실 문 있는 앞벽(복도쪽 低Z)을 뒤로 들여 앞 복도 확보 — 깊이 축소(단일 출처)
    placeMark(levels[2], true, wcW3, liftD - wcSetback3);   // 3층 화장실 = 뒤쪽벽 따라 wcW3(X) × 왼쪽벽 따라 liftD−0.2(Z, 앞벽 0.2m 들임)
  ...
  captureInto(s2Wall3Objects, () => {
    ...
    const ecuRoomZ = liftZ0 + 0.2;                                 // 실외기실 高Z 벽 = 화장실 앞벽(단일 출처)
