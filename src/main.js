...
// 부채꼴 코너 계단 — 앞·왼쪽 직선 계단 사이를 부채꼴(사분원)로 연결. (1단계: 첫 계단=가장 낮은 단 발판 높이의 부채꼴 프레임만, 다리·디딤판 없음)
{
  ...
  골조Objects.push(box({ x: exTop - legW / 2, z: (cz - bw / 2) - legW / 2, w: legW, d: legW, y: baseY, h: legTop2 - baseY, mat, cast: false }));   // +X쪽 끝 기둥
  골조Objects.push(box({ x: (cx + bw / 2) - legW / 2, z: ezTop - legW / 2, w: legW, d: legW, y: baseY, h: legTop2 - baseY, mat, cast: false }));   // −Z쪽 끝 기둥
}
...
function setPlanView() {
  ...
  corner.normalize().multiplyScalar(9.0);                     // 남쪽으로 더 물러나 비스듬히(입체감) 내려다봄
  camera.position.set(cx + corner.x, 15.5, cz + corner.y);    // 높이를 낮춰 수직 부감 → 약 30° 기운 조감
  controls.update();
}
