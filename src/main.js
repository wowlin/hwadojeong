...
const s2Floor2SlabT = 0.6;                  // 2층 바닥(전이보 포함) 슬래브 두께 — 치수·외벽 단일 출처
...
const _wBase = groundTopY + MAT_H, _wFh1 = 3.3, _wFh = 3.0, _wFh3 = 2.0;     // 1층 3.3 · 2층 3.0 · 3층 2.0(손님방 외벽 최저 1.8 = 바닥마감 0.2 뺀 값)
...
(() => {
  ...
  captureInto(s2Floor2Objects, () => {
    ...
    {
      const fy = levels[1], px1 = inX1, pz1 = inZ1, bx0 = far2 + 1.3, bz0 = zB0;
      ...
      // 샤워부스 — 안방쪽-앞(高X·低Z) 코너. 변기와 함께 왼쪽(안방쪽) 습식존. 0.9×0.85 방수트레이 + 열린 두 면(-X·+Z)에 유리.
      const shW = 0.9, shD = 0.85, shx = px1 - shW, shz = bz0;
      box({ x: shx, z: shz, w: shW, d: shD, y: fy, h: 0.06, mat: materials.shower });                       // 방수 트레이
      box({ x: shx, z: shz, w: 0.04, d: shD, y: fy, h: 2.0, mat: materials.glass });                        // -X 유리벽
...
//   층 경계(슬래브 밑면)에서 끊어 그려, 인접 층 외벽이 위·아래로 정확히 맞물림(켜면 연결).
{
  ...
  const rectWalls = (y0, y1, frontOpen, rightOpen, backOpen, leftOpen) => {
    ...
    if (backOpen) {   // 뒤벽 왼쪽(高x)에 폴딩 개구부 — 하부 띠·상부 인방·양쪽 기둥만 남김
      ...
      box({ x: bx, z: bz, w: a0 - bx, d: t, y: sillY, h: headY - sillY, mat: EW });                // 우측(低x) 벽 남김
  ...
  captureInto(s2Wall1Objects, () => {                                     // 우측벽·뒤벽 4짝 폴딩도어 — 정면 짝폭(0.68)·높이(2.4) 기준, 한 방향 아코디언(열린 상태)
    ...
    { const zc = s2BackZ - t / 2, syB = bO.sillY, hyB = bO.headY;
      ...
      const pw = (bO.a1 - bO.a0) / 4, mullW = 0.05, trk = 0.03;                                            // 4짝·짝폭 0.68·트랙 오프셋
      box({ x: bO.a0, z: zc - 0.06, w: bO.a1 - bO.a0, d: 0.12, y: syB, h: 0.08, mat: fdFrame });           // 하부 레일(2트랙 전폭)
      box({ x: bO.a0, z: zc - 0.06, w: bO.a1 - bO.a0, d: 0.12, y: hyB - 0.08, h: 0.08, mat: fdFrame });    // 상부 레일(2트랙 전폭)
      const pane = (x0, zt, mat) => {                                                                       // 유리 짝 + 좌우 세로살
        box({ x: x0, z: zt - 0.025, w: pw, d: 0.05, y: syB, h: hyB - syB, mat, cast: false });
        box({ x: x0, z: zt - 0.035, w: mullW, d: 0.07, y: syB, h: hyB - syB, mat: fdFrame, cast: false });
        box({ x: x0 + pw - mullW, z: zt - 0.035, w: mullW, d: 0.07, y: syB, h: hyB - syB, mat: fdFrame, cast: false });
      };
      pane(bO.a0, zc - trk, slGlass);            // 고정 바깥 좌 (앞트랙)
      pane(bO.a1 - pw, zc - trk, slGlass);        // 고정 바깥 우 (앞트랙)
      pane(bO.a0, zc + trk, slMove);              // 미닫이 좌 → 좌로 갈라져 바깥 뒤로 겹침 (뒤트랙)
      pane(bO.a1 - pw, zc + trk, slMove);          // 미닫이 우 → 우로 갈라져 바깥 뒤로 겹침 (뒤트랙)
      box({ x: bO.a0 + pw - 0.06, z: zc + trk - 0.13, w: 0.045, d: 0.045, y: syB + 0.95, h: 0.28, mat: materials.handle });   // 좌 미닫이 손잡이(가운데쪽)
      box({ x: bO.a1 - pw + 0.02, z: zc + trk - 0.13, w: 0.045, d: 0.045, y: syB + 0.95, h: 0.28, mat: materials.handle });    // 우 미닫이 손잡이(가운데쪽)
      label(`1층 뒤 슬라이드창 ${fmtDim(bO.a1 - bO.a0)}×${fmtDim(hyB - syB)}m (4짝 양미서기·짝폭 0.8·가운데 열림)`, (bO.a0 + bO.a1) / 2, syB + 1.0, s2BackZ + 0.3, 'opening'); }
...
//   처마=3층 벽 상단(roofY) 밑선. 두께 260mm(단열 260T) + 징크 마감. 처마 앞뒤 1.0m·좌우 0.4m.
{
  const sideOver = 0.4, eaveOver = 1.0, thk = roofThickness, zf = 0.05, tan = Math.tan(s2RoofPitch);
  ...
  captureInto(s2Roof3Objects, () => {
    ...
    label('지붕: 단열 260T + 리얼징크 · 박공 32° · 처마 앞뒤 1.0m·좌우 0.4m', s2W / 2, topRidgeY + 0.45, s2RidgeZ - 1.4, 'struct');
