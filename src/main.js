...
const S2_STAIR = { T: 0.27, R: 0.15, W: 0.75, g: 0.1, tTh: 0.06, nosing: 0.02, rTh: 0.03, usTh: 0.04, nUpper: [9, 9], slabT: floorFinishH, floorH: [3.0, 3.0] };  // slabT=1층 층참=바닥 마감 두께(콘크리트 기초 위 부자재+포세린, floorFinishH 0.20). 층고 1·2층 모두 3.0m(천장고 2.7+슬래브 0.3). nosing=계단코·rTh=챌판두께·usTh=계단아래문두께·nUpper=상부런 비행별 단수(그리기·메모 단일 출처)
...
(() => {
  ...
  const { T, R, W, g, tTh, nosing, rTh, usTh, nUpper } = S2_STAIR;
...
const NOTES = {
  ...
  get s2StairF1() {                                        // 계단 사양 + 1층 계단참 아래 옷장 — 모두 계단 상수서 자동 계산
    const { T, R, W, g, tTh, floorH, nosing, rTh, usTh, nUpper } = S2_STAIR;
    const wF = 2 * W + g;                                  // 계단참 깊이(두 런 + 틈)
    const n1 = Math.round(floorH[0] / R), n2 = Math.round(floorH[1] / R);   // 비행별 단 수
    const nL = n1 - 2 - nUpper[0];                         // 1→2 하부런 단 수(상부런 수는 S2_STAIR.nUpper)
