...
function planHDim(fixed, a, b, text) {
  lengthDim('x', fixed, a, b, text, { labelDist: 0, side: 1, y: 0.012, h: 0.002, labelLift: 0.05 });   // 라벨은 치수선 중앙. y는 바닥 가까이(시차 제거), 깊이무시로 안 묻힘
}
...
function planVDim(fixed, a, b, text) {
  ...
  label(text, fixed, 0.05, Math.max(a, b) + 0.5, 'dim');   // 세로선 위쪽 끝(+Z 머리 위)에 카드. 위치는 평면 offset(시차無), y는 바닥 가까이(시차 제거), 깊이무시로 안 묻힘
}
...
// 부채꼴 코너 계단 — 앞·왼쪽 직선 계단 사이를 부채꼴(사분원)로 연결. (1단계: 첫 계단=가장 낮은 단 발판 높이의 부채꼴 프레임만, 다리·디딤판 없음)
{
  ...
  const arc = (R, yTop) => { const N = 6; for (let j = 0; j < N; j += 1) { const a0 = j * (Math.PI / 2) / N, a1 = (j + 1) * (Math.PI / 2) / N; const [x0, z0] = P(R, a0), [x1, z1] = P(R, a1); barXZ(x0, z0, x1, z1, yTop); } };
  // 첫 계단(가장 낮은 단) 부채꼴(피자조각) 테두리 — 중심(코너)에서 뻗는 곧은 변 2개 + 바깥 호. 코너 바깥 빈 사분면에만 그려 직선 계단과 안 겹침.
  // 두 곧은 변은 데크 모서리선(x=cx / z=cz) '안쪽(사분면)'으로 막대 두께만큼만 들여 직선 계단 막대와 면이 닿되 겹치지 않게. 높이·굵기·바깥 반경 모두 직선 1계단과 동일 → 튀어나옴 없음.
  const R = 2 * tread;                                          // 바깥 반경 = 직선 1계단 바깥 끝과 동일
  const frameTopY = (topY - 2 * rise) - deckFinishT;            // 직선 1계단 틀 막대 윗면과 동일 레벨(발판 표면 바로 아래)
  골조Objects.push(box({ x: cx, z: cz - bw, w: R, d: bw, y: frameTopY - bw, h: bw, mat, cast: false }));   // +X 곧은 변(z=cz 안쪽)
  골조Objects.push(box({ x: cx, z: cz - R, w: bw, d: R, y: frameTopY - bw, h: bw, mat, cast: false }));    // −Z 곧은 변(x=cx 안쪽)
  arc(R, frameTopY);                                            // 바깥 호
}
