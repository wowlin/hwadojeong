...
// 거실 전면: 안방과 동일한 출입 가능 샷시(데크로 통하는 전면 도어창) — 상부 인방만
...
// 후면 벽: 거실은 싱크대용 창(전면에서 이동), 안방 후면 창
...
label('주방 독일식 시스템도어 VATON PS', livingYardSashX + yardSashW / 2, yardSashTopY + 0.14, buildingFrontZ - 0.35, 'opening');
...
// 공유 좌표는 여기서 한 번 계산해 세 그룹이 공유(단일 출처).
{
  ...
  captureInto(atticInnerWallObjects, () => {
    ...
    horizontalWallWithGaps(stairLowXRunX, stairOpeningStart, stairHighXWallX - stairLowXRunX, secondWallY, [[atticStairDoorX, atticStairDoorX + interiorDoorW]], secondAtticFrontWallH, interiorWall);   // 입구 가로벽 거실측 = 거실 벽면(stairLowXRunX)에 정렬
...
//  roofLowX/roofW로 X 범위를 지정해 거실 앞·안방 앞에 같은 형식으로 각각 설치한다.
...
captureInto(dimObjects, () => {
  ...
  // 가로 — 위쪽: 기초 8.5 / 안방 측백 0.5 (거실 0.5는 아래쪽으로 이동)
  ...
  // 세로 — 안방(왼쪽) 건물 깊이 4 / 거실(오른쪽) 뒤 이격 합 1m + 건물 깊이 4 + 데크 깊이
...
firstFloorObjects.push(...scene.children.slice(_firstFanStart));   // 1층 거실·안방 실링팬을 1층 그룹에 추가
...
// 1층 — 벽걸이 에어컨용 콘센트(거실 좌측 외벽 상부). 송풍을 +X로 보내 계단실 앞을 지나 안방까지.
...
const PARTS = [
  ...
  { key: 'stair',      arrays: [stairCoreObjects, livingInnerWallObjects, familyInnerWallObjects] },   // 거실측 벽·안방 내력벽을 계단 토글에 합침
...
const NOTES = {
  ...
  get s2Floor2() {                                         // 2층 — 화장실·앞방 크기. 그리기 실좌표(s2Geo)·모듈 상수서 자동 계산
    ...
    return { title: '2층 — 화장실 · 안방', body: [
      ...
      '',
      `[화장실 배치] 좌(안방쪽)=습식·우(거실쪽)=건식 분리. 문=앞 분리벽(안방서 밀어 +Z, 폭 ${fmtDim(s2F2.doorW)}, 도착칸쪽 벽서 ${fmtDim(s2WallInner)}). 앞벽 ${fmtDim(s2WallInner)} m.`,
  ...
  get s2Floor3() {                                         // 3층 — 계단앞(계단실 단열) 문 요구사항. 계단 상수·박공 단면서 자동 계산
    ...
    return { title: '3층 — 계단앞 문(계단실 단열)', body: [
      ...
      `- 변기↔세면대 중심 간격 ${fmtDim(s2WcSinkGap)} m(주택 욕실 표준). 거실쪽 벽·문은 복도쪽으로 더 빼 문이 열려도 세면대에 안 닿게 ${fmtDim(s2WcSinkClear)} m 여유.`,
...
function drawStairAnno(p) {
  ...
  captureInto(loftPassAnno, () => label(`다락 통행 ${fmtDim(loftPass)}m`, laneB + W / 2, loftY + 0.22, insideZ0 + loftPass / 2, 'dim'));
  ...
  room({ x: firstLivingX, z: insideZ0, w: firstLivingW, d: firstLivingD, y: roomY, mat: materials.living, text: roomText('주방', firstLivingW, firstLivingD) });
