...
function planHDim(fixed, a, b, text) {
  lengthDim('x', fixed, a, b, text, { labelDist: 0, side: 1, y: 0.012, h: 0.002, labelLift: 0.35 });
}
...
function planVDim(fixed, a, b, text) {
  ...
  label(text, fixed, 0.35, Math.max(a, b) + 0.5, 'dim');   // 세로선 위쪽 끝(+Z 머리 위)에 카드, 선과 직교 겹침 피함. 공중(y=0.35)
}
...
// 부채꼴 코너 계단 — 앞·왼쪽 직선 계단 사이를 부채꼴(사분원)로 연결. (1단계: 첫 계단=가장 낮은 단 발판 높이의 부채꼴 프레임만, 다리·디딤판 없음)
{
  ...
  const mat = materials.deckStairFrame;   // 데크틀 색
  ...
  // 첫 계단(가장 낮은 단) = 직선 1계단과 같은 높이·각관 레벨(반경 tread~2·tread 부채꼴 띠). 방사 양변 2개 + 안·바깥 호.
  const r0 = tread, r1 = 2 * tread;
  const frameTopY = (topY - 2 * rise) - deckFinishT;            // 직선 1계단 틀 막대 윗면과 동일 레벨(발판 표면 바로 아래)
  barXZ(...P(r0, 0), ...P(r1, 0), frameTopY);                   // 왼쪽 직선 계단과 잇는 방사 변(0°)
  barXZ(...P(r0, Math.PI / 2), ...P(r1, Math.PI / 2), frameTopY);   // 앞 직선 계단과 잇는 방사 변(90°)
  arc(r0, frameTopY);                                           // 안쪽 호
  arc(r1, frameTopY);                                           // 바깥 호
}
