...
function planHDim(fixed, a, b, text) {
  lengthDim('x', fixed, a, b, text, { labelDist: 0, side: 1, y: 0.012, h: 0.002, labelLift: 0.1 });   // 라벨은 치수선 중앙(labelDist 0), y축으로 치수선보다 살짝 위(labelLift 0.1)
}
...
// 부채꼴 코너 계단 — 앞·왼쪽 직선 계단 사이를 부채꼴(사분원)로 연결. (1단계: 첫 계단=가장 낮은 단 발판 높이의 부채꼴 프레임만, 다리·디딤판 없음)
{
  ...
  fan(2 * tread, baseY + bw);                                  // 계단 바닥(지면 베이스): 첫 계단과 같은 부채꼴(바깥 반경 2·디딤), 지면 높이
  fan(2 * tread, (topY - 2 * rise) - deckFinishT);             // 첫 계단(가장 낮은 단): 바깥 반경 2·디딤, 낮은 단 높이
  fan(1 * tread, (topY - 1 * rise) - deckFinishT);             // 두번째 계단(데크 옆 높은 단): 바깥 반경 1·디딤, 높은 단 높이
}
